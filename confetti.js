...
(function() {
	...
	function startConfetti(timeout, _min = false, _max = false) {
		...
		if (canvas === null) {
			...
			context = canvas.getContext("2d");
    const values = [_min, _max, confetti.maxCount].filter(Number.isInteger);
    const max = Math.max(...values);
    const min = Math.min(...values);
		const count = particles.length + ((Math.random() * (max - min) + min) | 0);
